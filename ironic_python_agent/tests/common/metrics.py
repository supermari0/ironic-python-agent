...
class TestMetricLogger(test_base.BaseTestCase):
    ...
    def test_histogram(self):
        self.ml.histogram('metric', 10)
        self.ml._histogram.assert_called_once_with(
            'prefix.metric.histogram', 10)
    ...
    @mock.patch('ironic_python_agent.common.metrics.MetricLogger.counter')
    @mock.patch('ironic_python_agent.common.metrics.MetricLogger.histogram')
    def test_instrument(self, mock_histogram, mock_counter):
        ...
        mock_counter.assert_called_once_with(('foo', 'bar', 'baz'), 1)
        # TODO(Alex Weeks): Sanity-check the histogram value
        mock_histogram.assert_called_once_with(('foo', 'bar', 'baz'), mock.ANY)
...
class TestStatsdMetricLogger(test_base.BaseTestCase):
    ...
    @mock.patch('ironic_python_agent.common.metrics.StatsdMetricLogger._send')
    def test_histogram(self, mock_send):
        self.ml._histogram('metric', 10)
        mock_send.assert_called_once_with('metric', 10, 'h')
