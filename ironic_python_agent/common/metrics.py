...
class MetricLogger(object):
    ...
    def histogram(self, m_parts, m_value):
        """Send histogram data."""
        self._histogram(self._getMetricName(m_parts) + '.histogram', m_value)
    ...
    @abc.abstractmethod
    def _histogram(self, m_parts, m_value):
        """Abstract method for backends to implement histogram behavior."""
    ...
    def instrument(self, *parts):
        ...
        def decorator(f):
            ...
            def wrapped(*args, **kwargs):
                self.counter(parts, 1)

                start = time.time()
                result = f(*args, **kwargs)
                duration = time.time() - start

                # Log the timing data as a histogram (to make analysis easier)
                self.histogram(parts, duration)
                return result
            return wrapped
        return decorator

    def instrument_context(self, *parts):
        """Returns a context manager that instruments a function, bound to this
        MetricLogger.  For example:
        ...
        with METRICS.instrument_context('foo'):
            foo(bar, baz)
        """
        return InstrumentContext(self, '.'.join(parts))
...
class NoopMetricLogger(MetricLogger):
    ...
    def _histogram(self, m_name, m_value):
        pass
...
class StatsdMetricLogger(MetricLogger):
    ...
    def _histogram(self, m_name, m_value):
        return self._send(m_name, m_value, self.HISTOGRAM_TYPE)
...
class InstrumentContext(object):
    ...
    def __init__(self, logger, *parts):
        self.logger = logger
        self.parts = parts

    def __enter__(self):
        self.logger.counter(self.parts, 1)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        # Log the timing data as a histogram (to make analysis easier)
        self.logger.histogram(self.parts, duration)
