...
class RawPromiscuousSockets(object):
    ...
    def __enter__(self):
        for interface_name, sock, ifr in self.interfaces:
            ...
            try:
                ...
                LOG.debug('Binding interface for LLDP: %s', interface_name)
    ...
    def __exit__(self, exception_type, exception_val, trace):
        if exception_type:
            LOG.exception('Error while getting LLDP info: %(type)s: %(val)',
...
def _get_lldp_info(interfaces):
    ...
    while interfaces:
        LOG.info('Waiting on LLDP info for interfaces: %(interface)s, timeout:'
                 ' %(timeout)s', {'interfaces': interfaces,
        ...
        for s in r[0]:
            ...
            for index, interface in enumerate(interfaces):
                if s == interface[1]:
                    LOG.debug('Found LLDP info for interface: %s',
                              interface[0])
